...
SELECT @CUTOFF_VMSAMPLEID = max(sample_id) from vmware_perf_sample WHERE sample_time < @CUTOFF_DATE
...
SELECT @CUTOFF_NDSAMPLEID = max(id) from net_device_perf_sample WHERE sample_time < @CUTOFF_DATE
...
INSERT #SUPPORT_DBCLEANUP VALUES ('PERFORMANCE_DISK_TOTAL', 'SAMPLE_ID < ' + @CUTOFF_SAMPLEID , 0, 0, 0 )
