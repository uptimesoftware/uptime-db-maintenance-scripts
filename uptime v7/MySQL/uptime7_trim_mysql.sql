...
#-- deletes for net_device_perf tables added in 7
select min(id) into @net_device_ID from net_device_perf_sample where date (sample_time) like DATE_SUB(CURDATE(), INTERVAL @days DAY);
select @net_device_ID as 'net_device_ID', sample_time from net_device_perf_sample where id = @net_device_ID;

delete from net_device_perf_ping where sample_id < @net_device_ID;
delete from net_device_perf_port where sample_id < @net_device_ID;
